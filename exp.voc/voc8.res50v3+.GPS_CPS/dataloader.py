...
class VOC(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.unsupervised:
            img, gt = self._fetch_data(img_path, gt_path) # supervised
        else:
            img, gt = self._fetch_data(img_path, None) # unsupervised
    ...
    def _fetch_data(self, img_path, gt_path=None, dtype=None):
        img = self._open_image(img_path)
        if gt_path is not None: # supervised
            gt = self._open_image(gt_path, cv2.IMREAD_GRAYSCALE, dtype=dtype)
            return img, gt
        return img, None # unsupervised
