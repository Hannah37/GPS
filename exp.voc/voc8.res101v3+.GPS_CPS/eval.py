...
if __name__ == "__main__":
    ...
    model = SingleNetwork(config.num_classes, criterion=None, norm_layer=nn.BatchNorm2d) # a teacher or student
    # network = Network(num_classes=config.num_classes, criterion=None, norm_layer=nn.BatchNorm2d)

    data_setting = {'img_root': config.img_root_folder,
                    'gt_root': config.gt_root_folder,
                    'train_source': config.train_source,
                    'eval_source': config.eval_source}
    val_pre = ValPre() # preprocessing
    ...
    with torch.no_grad():
        segmentor = SegEvaluator(dataset, config.num_classes, config.image_mean,
                                 ...
                                 args.show_image)
        segmentor.run(config.pretrain_snapshot_dir, args.epochs, config.val_log_file,
