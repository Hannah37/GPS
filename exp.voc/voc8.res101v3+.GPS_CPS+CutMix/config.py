...
C.pretrain_dir = 'save/ss6_pre_res101_8.pt' # 73.216
C.t_dir = 'save/aug2_t_res101_8.pt'
C.b1_dir = 'save/aug2_b1_res101_8.pt'
C.b2_dir = 'save/aug2_b2_res101_8.pt'
...
C.repo_name = 'GPS_semiseg'
C.abs_dir = osp.realpath(".")
C.this_dir = C.abs_dir.split(osp.sep)[-1]

C.root_dir = C.abs_dir[:C.abs_dir.index(C.repo_name) + len(C.repo_name)]
# C.log_dir = osp.abspath('log')
C.log_dir = os.environ['snapshot_dir']
...
if os.getenv('snapshot_dir'):
    ...
    C.pretrain_snapshot_dir = osp.join(os.environ['snapshot_dir'], "pretrain_snapshot")
    C.b1_snapshot_dir = osp.join(os.environ['snapshot_dir'], "b1_snapshot")
    C.b2_snapshot_dir = osp.join(os.environ['snapshot_dir'], "b2_snapshot")
else:
...
C.val_log_file = C.log_dir + 'snapshot/epoch-last.pth'
C.link_val_log_file = C.log_dir + 'snapshot/epoch-last.pth'
...
# C.train_source = osp.join(C.dataset_path, 'pascal_voc', "subset_train_aug/train_aug_labeled_1-{}.txt".format(C.labeled_ratio))
# C.unsup_source = osp.join(C.dataset_path, 'pascal_voc', "subset_train_aug/train_aug_unlabeled_1-{}.txt".format(C.labeled_ratio))
# C.eval_source = osp.join(C.dataset_path, 'pascal_voc', "val.txt")
C.is_test = False
C.fix_bias = True
C.bn_eps = 1e-5
C.bn_momentum = 0.1
...
C.num_workers = 16 # set 0 for only 1 process
C.train_scale_array = [0.5, 0.75, 1, 1.5, 1.75, 2.0]
C.warm_up_epoch = 0

# fix teacher lr in step 3
C.ngpus = int(os.environ['NGPUS'])
C.fixed_lr = (C.lr * C.ngpus) * (1/(C.nepochs * C.niters_per_epoch)) ** C.lr_power # 7.32e-07
